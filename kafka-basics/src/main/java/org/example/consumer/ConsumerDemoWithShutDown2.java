...
public class ConsumerDemoWithShutDown2 {
    private static final Logger log =
            LoggerFactory.getLogger(ConsumerDemoWithShutDown2.class.getSimpleName());
    public static void main(String[] args) {
        ...
        // for the purpose of gracefully shutdown(exit loop)
        // Create reference to the main thread
        final Thread mainThread = Thread.currentThread();

        // click on the "exit" will run this hook in separate thread
        Runtime.getRuntime().addShutdownHook(new Thread() {
            @Override
            public void run() {
                log.info("Detected a consumer shutdown request, let's exit by calling consumer.wakeup...");

                // next time we do consumer.poll() it will throw a wakeup exception
                // some code will be executed after the while loop
                consumer.wakeup(); // throw exception next iteration of while loop!

                // join the main thread to allow the execution
                // of the rest of the code - code after the loop inside the try( in the main thread)
                try {
                    mainThread.join();
        ...
        // we need try because we know consumer with throw exception
        try {
            ...
            log.info("Expected exception...Consumer is starting to shut down");
        } catch (Exception e) { // expected!
            log.info("Unexpected exception in consumer", e);
        } finally {
            // close the consumer,
            // this will commit the offsets, close connection to Kafka,
            // allow the group to re-balance after this consumer leaves it
            consumer.close();
